...
@section('title', 'Gaji Tahfidz')

@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[hsl(var(--foreground))]">Data Penggajian Tahfidz</h1>
            <p class="text-sm text-[hsl(var(--muted-foreground))] mt-1">Kelola gaji bulanan guru tahfidz</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('tahfidz-payrolls.create') }}">
                <x-ui.button>
    ...
    <x-ui.card class="mb-6">
        <form method="GET" action="{{ route('tahfidz-payrolls.index') }}" class="flex flex-wrap items-end gap-4">
            <div class="space-y-2">
            ...
            <a href="{{ route('tahfidz-payrolls.report', ['month' => $selectedMonth, 'year' => $selectedYear]) }}" target="_blank">
            ...
            <a href="{{ route('tahfidz-payrolls.print_all', ['month' => $selectedMonth, 'year' => $selectedYear]) }}" target="_blank">
                <x-ui.button type="button" variant="outline">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="border border-[hsl(var(--border))] rounded-lg bg-[hsl(var(--card))] overflow-hidden" x-data="{ open: false }">
                ...
                <div x-show="open" x-collapse class="border-t border-[hsl(var(--border))]">
                    ...
                    <div class="px-4 py-3 bg-[hsl(var(--secondary))]/30 flex flex-wrap gap-2">
                        <a href="{{ route('tahfidz-payrolls.batch.edit', $batch) }}">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @foreach($batch->payrolls as $payroll)
                                        <tr>
                                            <td>
                                                <div class="font-medium text-[hsl(var(--foreground))]">{{ $payroll->teacher->name ?? 'N/A' }}</div>
                                                <div class="text-xs text-[hsl(var(--muted-foreground))]">{{ $payroll->teacher->position ?? 'Guru Tahfidz' }}</div>
                                            </td>
                                            <td class="text-center">{{ $payroll->teaching_hours }}</td>
                                            <td class="text-center">{{ $payroll->attendance_days }}</td>
                                            <td class="text-right font-semibold text-green-600">
                                                Rp {{ number_format($payroll->total_salary, 0, ',', '.') }}
                                            </td>
                                            <td class="text-right">
                                                <a href="{{ route('tahfidz-payrolls.show', $payroll) }}" target="_blank">
        ...
        @empty
            <x-ui.card>
                <div class="text-center py-8">
                    ...
                    <p class="text-[hsl(var(--muted-foreground))]">Belum ada data gaji tahfidz untuk periode ini.</p>
                    <a href="{{ route('tahfidz-payrolls.create') }}" class="inline-block mt-4">
                        <x-ui.button>Proses Gaji Tahfidz Baru</x-ui.button>
                    </a>
                </div>
            </x-ui.card>
        @endforelse
