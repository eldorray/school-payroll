...
@section('title', 'Proses Gaji Tahfidz')

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[hsl(var(--foreground))]">Proses Penggajian Tahfidz</h1>
        <p class="text-sm text-[hsl(var(--muted-foreground))] mt-1">Hitung dan simpan gaji bulanan guru tahfidz</p>
    </div>
    ...
    <x-ui.card>
        <div class="mb-6 p-4 rounded-lg bg-[hsl(var(--secondary))]">
            ...
            @php $settings = $activeYear->getSettingsForUnit(session('unit_id')); @endphp
            <p class="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                Tarif Standar: Mengajar (Rp {{ number_format($settings->teaching_rate_per_hour ?? 0, 0, ',', '.') }}/jam),
                Transport (Rp {{ number_format($settings->transport_rate_per_visit ?? 0, 0, ',', '.') }}/hari)
            </p>
        </div>

        @if ($teachers->isEmpty())
            <x-ui.alert variant="warning" class="mb-6">
                <p>Tidak ada guru tahfidz yang aktif. Silakan tandai guru sebagai "Tahfidz" di halaman <a
                        href="{{ route('teachers.index') }}" class="underline">Daftar Guru</a>.</p>
            </x-ui.alert>
        @else
            <form action="{{ route('tahfidz-payrolls.store') }}" method="POST">
                @csrf
                ...
                <div class="flex flex-wrap gap-4 mb-6">
                    ...
                    <div class="space-y-2 flex-1 min-w-[200px]">
                        <label class="text-sm font-medium text-[hsl(var(--foreground))]">Nama Batch <span
                                class="text-[hsl(var(--muted-foreground))] font-normal">(Opsional)</span></label>
                        <input type="text" name="batch_name" class="input" placeholder="Contoh: Gaji Tahfidz, dll">
                    </div>
                </div>
                ...
                <div class="table-wrapper mb-6">
                    <table class="table">
                        <thead>
                            <tr>
                                <th rowspan="2" class="align-middle">Guru Tahfidz</th>
                                <th rowspan="2" class="text-center align-middle">Transport<br><span
                                        class="text-xs font-normal">(Hari)</span></th>
                                <th colspan="4" class="text-center border-b border-[hsl(var(--border))]">Potongan</th>
                                <th rowspan="2" class="text-right align-middle">Tunjangan<br><span
                                        class="text-xs font-normal">(Est)</span></th>
                            </tr>
                            <tr>
                            ...
                            @foreach ($teachers as $teacher)
                                @php
                                    $allowanceTotal = \App\Models\TeacherAllowance::where('teacher_id', $teacher->id)
                                        ...
                                        ->sum('amount');
                                    $annualSetting = $teacher->annualSettings
                                        ->where('academic_year_id', $activeYear->id)
                                        ->first();
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <div class="text-xs text-[hsl(var(--muted-foreground))]">
                                            {{ $teacher->position ?? 'Guru Tahfidz' }}</div>
                                    ...
                                    <td class="text-center">
                                        <input type="number"
                                            name="attendance[{{ $teacher->id }}][deductions][late_count]"
